//var baseurl = 'http://localhost/airkomcms/public/';
var baseurl = 'https://airkomgroup.com/cms/public/';
...
$('#confirmDelete').on('show.bs.modal', function (e) {
		 ...
		 msg = $(e.relatedTarget).attr('data-msg');
...
$('#bike-start, #bike-end').on('change', function(){
	var start = parseInt($('#bike-start').val());
	var end = parseInt($('#bike-end').val());
	$('#distance-travelled').val(end - start);
	
	var percentage = parseFloat($('#amountone').attr('data-percentage'));
	distance = parseInt($('#distance-travelled').val());
	$('#amountone').val(distance * percentage);
 });
 

$('.printbtn').on('click',function(e){
	e.preventDefault();
	$("#printTable").printMe({ "path": ["https://airkomgroup.com/cms/public/css/style.css"], "title": "Airkom Group" });
})

$('#propect_name').on('change',function(){
    id = $('#propect_name option:selected').val();

    $.ajax({
          url: baseurl + 'spt/get-contact?contactid='+id,
          type: 'GET',
          dataType: 'json',
...
$('#rprospect_name').on('change',function(){
    id = $('#rprospect_name option:selected').val();

    $.ajax({
          url: baseurl + 'roadmap/get-city?contactid='+id,
          type: 'GET',
          dataType: 'json',
          success: function(data) {
              $('#rprospect_city').val(data.name);
			  console.log(data.name);
          },
          error: function(jqXHR, textStatus, errorThrown) {
              console.log(textStatus, errorThrown);
...
const container = document.querySelectorAll('.drag-inner-list');
var div_array = [...container]; // converts NodeList to Array
div_array.forEach(div => {

let ps = new PerfectScrollbar(div);
});


tinymce.init({
      selector: '#company_brief,#services,#payment,#tax_information'
    });